...
export default function DashboardPage() {
  ...
  const handleDeleteProcurement = async (e: React.MouseEvent, id: string, name: string) => {
    e.preventDefault()
    e.stopPropagation()
    ...
    try {
      ...
      fetch(`/api/procurements/${id}`, { method: 'DELETE' }).catch(() => {})
    } catch {}
  }
  ...
  for (const proc of procurements) {
    const items = Array.isArray(proc?.items) ? proc.items : []
    for (const item of items) {
      totalItemsCount++
      const offers = Array.isArray(item?.offers) ? item.offers : []
      const sortedOffers = [...offers].sort((a, b) => (b.buyingScore ?? 0) - (a.buyingScore ?? 0))
      const bestOffer = sortedOffers[0]
      const worstOffer = [...offers].sort((a, b) => (b.totalPrice ?? 0) - (a.totalPrice ?? 0))[0]

      if (bestOffer && bestOffer.totalPrice) {
      ...
      if (worstOffer && worstOffer.totalPrice) {
        totalMaxAlternativeSpend += worstOffer.totalPrice
      }

      for (const offer of offers) {
        if (offer && offer.trustScore != null && offer.trustScore > 0) {
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      <div className="inline-flex items-center justify-end gap-3">
                        ...
                        <button
                          type="button"
                          onClick={(e) => handleDeleteProcurement(e, proc.id, proc.name)}
                          title="Eliminar cotización"
                          className="text-[#8792a2] hover:text-red-500 transition-colors p-1 rounded hover:bg-red-50 dark:hover:bg-red-950/40 cursor-pointer"
                        >
